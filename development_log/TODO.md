...
1. 阶段 4C：回调机制复盘和文档补齐。
     - 完成标准：能说清楚 `ConnectionCallback`、`MessageCallback`、业务 `CloseCallback` 分别在哪里设置、保存和调用。
     - 验证方法：根据源码画出三条业务回调调用链，并说明关闭回调为什么必须在 `erase()` 前执行。
     - 当前状态：源码检查和运行验证已通过，等待完成架构文档、测试文档和开发日志补充。

## P2：后续应该做

1. 阶段 4D：逐步收口回调职责和生命周期边界。
     - 完成标准：网络层不再保留默认 echo 业务；监听 Channel 和客户端 Channel 的失败处理职责能够区分；回调裸指针的有效期有明确约定。
     - 验证方法：先完成最小设计，再分成多个小阶段修改和验证。

2. 阶段 5：多线程 Reactor 前置设计。
     - 完成标准：明确主 Reactor、子 Reactor、连接分配和跨线程唤醒机制。
     - 验证方法：阶段 4D 完成后先更新设计文档，不直接开始大规模编码。
